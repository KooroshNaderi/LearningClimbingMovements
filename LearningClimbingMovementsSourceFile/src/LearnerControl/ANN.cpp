...
void Connection::apply_weight_decay(float learning_rate, float weight_decay)
{
	weights_ *= 1.0f - learning_rate*weight_decay;
}
...
void MultiLayerPerceptron::copy(const MultiLayerPerceptron & other)
{
	...
	weight_decay_ = other.weight_decay_;
	...
	while (random_index.size() > 0) {
		...
		if (batch_idx >= minibatch_size) {
			for (Connection& conn : connections_) {
				...
				conn.apply_weight_decay(learning_rate_, weight_decay_);
				conn.clamp_weights(min_weight_, max_weight_);
			}
			batch_idx = 0;
		}
...
void MultiLayerPerceptron::bayesian_optimization(const float ** in, const float ** out, unsigned data_points, unsigned minibatch_size)
{
	...
	int minibatches = data_points / minibatch_size;

	if (minibatches*minibatch_size < data_points) {
		++minibatches;
	}

	const int output_data_dim = output_operation_->size_ / 2;

	for (int batch = 0; batch < minibatches; ++batch) {

		std::vector<const float*> input_ptrs;
		std::vector<std::unique_ptr<Eigen::VectorXf>> gradients;

		for (int sample = 0; sample < minibatch_size; ++sample) {

			int rand_idx = rand() % data_points;

			input_ptrs.push_back(in[rand_idx]);

			Eigen::VectorXf output;
			output.resize(output_operation_->size_);

			std::unique_ptr<Eigen::VectorXf> gradient = std::unique_ptr<Eigen::VectorXf>(new Eigen::VectorXf);
			gradient->resize(output_operation_->size_);

			run(in[rand_idx], output.data());

			Eigen::Map<const Eigen::VectorXf> true_output(out[rand_idx], output_data_dim);


			//Mean
			for (int i = 0; i < output_data_dim; ++i) {
				int std_i = i + output_data_dim;

				float stdev = output[std_i];

				if (stdev < 0.0f) {
					stdev *= -1.0f;
				}

				if (stdev < std::numeric_limits<float>::epsilon()) {
					stdev = std::numeric_limits<float>::epsilon();
				}

				(*gradient)[i] = -(true_output[i] - output[i]) / (stdev*stdev);
			...
			for (int i = 0; i < output_data_dim; ++i) {
				...
				float mean_diff = true_output[i] - output[i];

				(*gradient)[std_i] = - mean_diff * mean_diff / (stdev*stdev*stdev) + 2.0f / stdev;
				(*gradient)[std_i] *= sign;
		...
		std::vector<float*> output_ptrs = vector_to_ptrs(gradients);

		const bool is_gradient = true;

		train_adam((const float**)input_ptrs.data(), (const float**)output_ptrs.data(), minibatch_size, minibatch_size, is_gradient);
...
void MultiLayerPerceptron::write_to_file(std::string filename)
{
	...
	net_file << weight_decay_ << std::endl;
