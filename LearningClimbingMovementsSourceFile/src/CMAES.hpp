...
public:
	...
	void init_with_dimension(unsigned dimension){
		...
		selected_samples_fraction_ = (Scalar)0.5;
		cc_ = (Scalar)0.3333;
		cs_ = (Scalar)0.3333;
		c1_ = (Scalar)0.3333;
		cmu_ = (Scalar)0.3333;
	...
	void update(std::vector<ScoredData >& data_with_score, bool use_weights = true, const Vector& extern_mean = Vector::Zero(0)){
		...
		for (const ScoredData& datum : data_with_score){
		...
		unsigned last_non_inf = (unsigned)(selected_samples_fraction_*(Scalar)data_with_score_ptr.size()); // 0
	...
	std::pair<Vector,Matrix> decompositionForSampling(){
		...
		std::pair<Vector, Matrix> statistics;
		statistics.first = mean_;

		Eigen::JacobiSVD<Matrix> svd(cov, Eigen::ComputeFullU | Eigen::ComputeFullV);
		Matrix U = svd.matrixU();
		Matrix V = svd.matrixV();
		Vector signular_values = svd.singularValues();

		for (int i = 0; i < signular_values.size(); i++) {
			if (signular_values[i] < minimum_exploration_variance_) {
				signular_values[i] = minimum_exploration_variance_;
			}

		}

		float singular_min = signular_values.minCoeff();
		float singular_max = signular_values.maxCoeff();

		const float max_condition_number = 1000.0f;

		if (singular_max / singular_min > max_condition_number) {
			singular_min = (std::max)(singular_max/max_condition_number, minimum_exploration_variance_);
			
			for (int i = 0; i < signular_values.size(); i++) {
				if (signular_values[i] < singular_min) {
					signular_values[i] = singular_min;
				}
			}
		}

		signular_values = signular_values.cwiseSqrt();

		statistics.second = U*signular_values.asDiagonal()*V.adjoint();
