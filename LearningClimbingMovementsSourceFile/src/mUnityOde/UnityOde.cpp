...
extern "C"
{
	...
	LONG WINAPI mUnhandledExceptionFilter(PEXCEPTION_POINTERS pExceptionPtrs)
	{
...
void odeGeomPlaneGetParams(int geomId, float& _d)
{
	OdeThreadContext & c = contexts[s_getIdx];
	dVector4 dResult;
...
EXPORT_API void odeJointSetAMotorMode(int jointId, int mode)
{
	ITERATE_THREADS(i)
	{
		dJointID joint = contexts[i].joints[jointId];
...
EXPORT_API int odeJointGetAMotorMode(int jointId)
{
	OdeThreadContext & c = contexts[s_getIdx];
	int dMode = dJointGetAMotorMode(c.joints[jointId]);
	return dMode;
}
EXPORT_API void odeJointSetAMotorNumAxes(int jointId, int num)
{
	ITERATE_THREADS(i)
	{
		dJointID joint = contexts[i].joints[jointId];
...
EXPORT_API int odeJointGetAMotorNumAxes(int jointId)
{
	OdeThreadContext & c = contexts[s_getIdx];
	int dNum = dJointGetAMotorNumAxes(c.joints[jointId]);
	return dNum;
}
EXPORT_API void odeJointSetAMotorAxis(int jointId, int anum, int rel, float x, float y, float z)
{
	ITERATE_THREADS(i)
	{
		dJointID joint = contexts[i].joints[jointId];
...
EXPORT_API void odeJointGetAMotorAxis(int jointId, int anum, float &x, float &y, float &z)
{
	OdeThreadContext & c = contexts[s_getIdx];
	dVector3 dResult;
...
EXPORT_API int odeJointGetAMotorAxisRel(int jointId, int anum)
{
	OdeThreadContext & c = contexts[s_getIdx];
	int dRel = dJointGetAMotorAxisRel(c.joints[jointId], anum);
	return dRel;
}
EXPORT_API void odeJointSetAMotorAngle(int jointId, int anum, float angle)
{
	ITERATE_THREADS(i)
	{
		dJointID joint = contexts[i].joints[jointId];
...
EXPORT_API float odeJointGetAMotorAngle(int jointId, int anum)
{
	OdeThreadContext & c = contexts[s_getIdx];
	dReal dAngle = dJointGetAMotorAngle(c.joints[jointId], anum);
	return static_cast<float>(dAngle);
}
EXPORT_API float odeJointGetAMotorAngleRate(int jointId, int anum)
{
	OdeThreadContext & c = contexts[s_getIdx];
	dReal dAngleRate = dJointGetAMotorAngleRate(c.joints[jointId], anum);
	return static_cast<float>(dAngleRate);
}
EXPORT_API void odeJointGetAMotorAnglesFromBodyRotations(int jointId, ConstOdeQuaternion q1, ConstOdeQuaternion q2, OdeVector result)
{
	OdeThreadContext & c = contexts[s_getIdx];
	dJointID joint = c.joints[jointId];
...
EXPORT_API ConstOdeVector odeJointGetAccumulatedTorque(int jointId, int bodyIdx)
{
	OdeThreadContext & c = contexts[s_getIdx];
...
EXPORT_API ConstOdeVector odeJointGetAccumulatedForce(int jointId, int bodyIdx)
{
	OdeThreadContext & c = contexts[s_getIdx];
	dJointFeedback * feedback = dJointGetFeedback(c.joints[jointId]);
...
EXPORT_API int odeRaycast(float px, float py, float pz, float dx, float dy, float dz, float length, OdeVector out_pos, float & out_depth, unsigned long collideBits, unsigned long categoryBits)
{
	OdeThreadContext & c = contexts[s_getIdx];
...
EXPORT_API int odeRaycastGeom(float px, float py, float pz, float dx, float dy, float dz, float length, OdeVector out_pos, float & out_depth, unsigned long collideBits, unsigned long categoryBits)
{
	OdeThreadContext & c = contexts[s_getIdx];
